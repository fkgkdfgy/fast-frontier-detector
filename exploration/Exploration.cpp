...
Exploration::Exploration(int  mapSizeX, int mapSizeY, char* logFile) : _scanList(mapSizeX, mapSizeY){
	_flgFirstTime = true;
	_mapSizeX = mapSizeX;
	_mapSizeY = mapSizeY;
...
void Exploration::unMarkFrontier(const POINT_LIST &frontier) {
...
POINT Exploration::findBestCentroid(FRONTIER_LIST &frontiers, int** map, int width, int height) {
...
POINT Exploration::findBestCentroidGUI(FRONTIER_LIST &frontiers) {
...
void Exploration::setCurrentPos(POINT &currPosition) {
	_currentPos = currPosition;
}

POINT Exploration::calcNext(POINT &currPosition, int **map, int width, int height) {
...
POINT Exploration::calcNext(POINT &currPosition, const MAP &map) {
 ...
 */
POINT Exploration::calcCentroid(const POINT_LIST &points) {
...
list<POINT_LIST > Exploration::findFrontiers(POINT &root, int** map, int width, int height) {
...
list<POINT_LIST > Exploration::findFrontiers(POINT &root, const MAP &map) {
...
bool Exploration::isNeighborsRelevant(POINT &p, const MAP &map) {
...
bool Exploration::isNeighborsRelevant(POINT &p, int** map, int width, int height) {
...
POINT_LIST Exploration::getFrontier(POINT &root, int **map, int width, int height) {
...
POINT_LIST Exploration::getFrontier(POINT &root, const MAP &map) {
	POINT_LIST frontier;	// collection of frontier points
...
double Exploration::realDistance(POINT& target, int** map, int width, int height) {
...
int Exploration::getCellData(POINT &p, int** map, int width, int height) {
...
bool Exploration::isFrontierPoint(POINT &p, const MAP &map) {
...
bool Exploration::isFrontierPoint(POINT &p, int **map, int width, int height) {
...
bool Exploration::isInList(const POINT &p, ScanState state) {

	return _scanList.isState(p.x, p.y, state);
}

inline void Exploration::addToList(const POINT &p, ScanState state) {
...
void Exploration::removeFromList(const POINT &p, ScanState state) {
...
bool Exploration::isInside(int x, int y, int width, int height) {
	if (x < 0 || y < 0)
		return false;
...
void Exploration::printPoint(const POINT &p) {
	cout << "(" << p.x << "," << p.y << ")" ;
}

bool Exploration::isOpenSpace(POINT &p, const MAP &map) {
...
//void Exploration::printMapValues(PointHashMap &map) {
...
list<POINT> Exploration::getNeighbors(POINT &p, const MAP &map) {
...
list<POINT> Exploration::getNeighbors(int &x, int &y, int &width, int &height) {
	list<POINT> neighbours;
...
vector<POINT_LIST> Exploration::getDetectedFrontiers() {
...
vector<Point> Exploration::planPath(POINT &dest, int** map, int width, int height) {
