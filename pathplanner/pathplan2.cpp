...
//    			// If cell is free, check its neighbours
//    			if (map[y*width+x] != 0) {
//    				for (int ix = -1; ix <= 1; ix++)
//    					for (int iy = -1; iy <= 1; iy++)
//    						// If neighbour cell is occupied, increase its weight
//    						if (map[(y+iy)*width+(x+ix)] == 0) {
//    							cout << "b";
//    							tmap[y*width+x] = 2;
//    							continue;
//    						}
...
//    	}
    ...
    if(map[tx+ty*w]==0) { cout << "Target unreachable" << endl; return graph; }
    if(map[sx+sy*w]==0) { cout << "Source unreachable" << endl; return graph; }
        ...
        for (int x = -1; x < 2; ++x)
        	for (int y = -1; y < 2; ++y) {
        		if (x == 0 && y == 0)
        		 ...
        		 node myNode = expand(sx, sy, sx + x, sy + y, A.G);
        		if (myNode.used) act.push_back(myNode);
        	}
